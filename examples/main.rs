...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    router.add_route(Route::new(
        "GET",
        "/",
        |_req: Request<Body>, plugins: Arc<PluginManager>| async move {
            // Call AI plugin
            if let Some(plugin) = plugins.get("ai") {
                if let Some(ai_plugin) = plugin.as_any().downcast_ref::<AIPlugin>() {
                    let ai_response = ai_plugin
                        .call_ai("Hello from AIVIANIA!")
                        .await
                        .unwrap_or_else(|_| "AI Error".to_string());
    ...
    router.add_route(Route::new(
        "POST",
        "/echo",
        |req: Request<Body>, _plugins: Arc<PluginManager>| async move {
    ...
    router.add_route(Route::new(
        ...
        |req: Request<Body>, plugins: Arc<PluginManager>| async move {
            ...
            Response::new(StatusCode::INTERNAL_SERVER_ERROR)
                .body(Body::from("WebSocket plugin not found"))
        },
    ));
