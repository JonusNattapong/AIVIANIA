...
async fn integration_register_login_rbac() {
    ...
    plugin_mgr.add(Box::new(AuthService::new("integration-secret-key-which-is-long")));
    let plugins = Arc::new(plugin_mgr);

    let server = AivianiaServer::new(router).with_plugin(Box::new(DatabasePlugin::new(db.clone())))
        .with_plugin(Box::new(AuthService::new("integration-secret-key-which-is-long")));
    ...
    // Register a user
    let register_resp = client.post(&format!("http://{}/register", addr))
        .json(&json!({"username": "itest", "password": "password123"}))
        .send().await.expect("register request");
    ...
    // Login
    let login_resp = client.post(&format!("http://{}/login", addr))
        .json(&json!({"username": "itest", "password": "password123"}))
    ...
    // Access user profile
    let profile_resp = client.get(&format!("http://{}/user/profile", addr))
        .header("Authorization", format!("Bearer {}", token))
        .send().await.expect("profile request");
    assert_eq!(profile_resp.status(), StatusCode::OK);

    // Admin access should be forbidden
    let admin_resp = client.get(&format!("http://{}/admin/dashboard", addr))
        .header("Authorization", format!("Bearer {}", token))
        .send().await.expect("admin request");
    assert_eq!(admin_resp.status(), StatusCode::FORBIDDEN);

    // Create admin user directly in DB
    let pw = Database::hash_password("adminpass").expect("hash");
    let admin_id = db.create_user("admin_integration", &pw).await.expect("create admin");
    db.assign_role_to_user(admin_id, "admin").await.expect("assign role");

    // Login as admin
    let login_admin_resp = client.post(&format!("http://{}/login", addr))
        .json(&json!({"username": "admin_integration", "password": "adminpass"}))
        .send().await.expect("login admin");
    assert_eq!(login_admin_resp.status(), StatusCode::OK);
    let login_admin_json: serde_json::Value = login_admin_resp.json().await.expect("parse admin login");
    let admin_token = login_admin_json["token"].as_str().expect("admin token");

    // Access admin dashboard
    let admin_ok = client.get(&format!("http://{}/admin/dashboard", addr))
        .header("Authorization", format!("Bearer {}", admin_token))
        .send().await.expect("admin ok");
