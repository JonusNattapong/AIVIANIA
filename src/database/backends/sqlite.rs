...
use tokio_rusqlite::{params_from_iter, Connection, Row};
    ...
    fn row_to_hashmap(row: &Row) -> Result<HashMap<String, Value>, rusqlite::Error> {
        ...
        for i in 0..column_count {
            ...
            let value = match row.get_ref(i).map_err(|e| rusqlite::Error::from(e))? {
                ...
                tokio_rusqlite::types::ValueRef::Real(f) => {
                    Value::Number(serde_json::Number::from_f64(f).unwrap())
                }
                tokio_rusqlite::types::ValueRef::Text(s) => Value::String(
                    String::from_utf8(s.to_vec()).map_err(|_| rusqlite::Error::InvalidQuery)?,
                ),
                tokio_rusqlite::types::ValueRef::Blob(b) => {
                    Value::Array(b.iter().map(|&b| Value::Number(b.into())).collect())
                }
            };
    ...
    fn convert_params(params: &[Value]) -> Vec<tokio_rusqlite::types::Value> {
        params
            ...
            .map(|param| match param {
                ...
                Value::Object(_) => tokio_rusqlite::types::Value::Text(
...
impl DatabaseConnection for SqliteConnection {
    async fn execute(&self, query: &str, params: Vec<Value>) -> Result<QueryResult, DatabaseError> {
        ...
        let rows_affected = self
            .conn
            .call(move |conn| Ok(conn.execute(&query, params_from_iter(params))?))
            .await
    ...
    async fn query(&self, query: &str, params: Vec<Value>) -> Result<QueryResult, DatabaseError> {
        ...
        let rows = self
            .conn
            .call(
                move |conn| -> Result<Vec<HashMap<String, Value>>, tokio_rusqlite::Error> {
                    let mut stmt = conn.prepare(&query)?;
                    let rows =
                        stmt.query_map(params_from_iter(params), |row| Self::row_to_hashmap(row))?;
    ...
    async fn query_one(
        ...
        let row = self
            .conn
            .call(
                move |conn| -> Result<Option<HashMap<String, Value>>, tokio_rusqlite::Error> {
                    let mut stmt = conn.prepare(&query)?;
                    let mut rows =
                        stmt.query_map(params_from_iter(params), |row| Self::row_to_hashmap(row))?;
